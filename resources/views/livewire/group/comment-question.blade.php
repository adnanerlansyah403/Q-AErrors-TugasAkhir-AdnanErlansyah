<div>
    <p class="text-md mt-10">Jawaban <span class="span">({{ !empty($question->comments) ? count($question->comments) : 0 }})</span></p>
    <form wire:submit.prevent="storeComment" action="#" class="mt-10">
        
    
    {{-- @if(session()->has('scs'))
        <div class="alert alert-success active" x-ref="alert">
            <a id="dismissAlertComment" class="cursor-pointer font-semibold text-white">
                Dismiss
            </a>  
            ...
            <script type="text/javascript">
                ...
                let dismissAlertComment = document.getElementById("dismissAlertComment");
        
                dismissAlertComment.addEventListener("click", function(event) {
    ...
    @endif --}}
        ...
        <div class="mb-6 w-full" id="comment">
            ...
            <div class="shadow-[rgba(60,_64,_67,_0.3)_0px_1px_2px_0px,_rgba(60,_64,_67,_0.15)_0px_2px_6px_2px] p-4 rounded-lg active:border border-red-primary @error('comment') border outline-red-primary @enderror transition duration-200">
                <textarea wire:model="comment" id="comment" rows="10" placeholder="Your comment..." class="text-md w-full"></textarea>
            </div>
            @error('comment') 
