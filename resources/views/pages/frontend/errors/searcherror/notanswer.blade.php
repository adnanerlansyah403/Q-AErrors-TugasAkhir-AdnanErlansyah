        ...
        <div class="flex items-center gap-10">
            <div class="flex items-center gap-6 px-6 py-7 rounded-lg shadow-[rgba(60,_64,_67,_0.3)_0px_1px_2px_0px,_rgba(60,_64,_67,_0.15)_0px_1px_3px_1px] w-max">
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route("errors.searcherror.index") }}" class="{{ $currentRoute == 'errors.searcherror.index' ? 'bg-slate-800' : '' }} font-bold px-6 outline outline-1 outline-red-primary py-2 rounded-lg hover:bg-slate-800 hover:text-white transition duration-200 hover:outline-none" style="{{ $currentRoute == 'errors.searcherror.index' ? 'color: white;' : '' }}">
                        <span class="span">N</span>ew
                    </a>
                    <a href="{{ route("errors.searcherror.notanswer.index") }}" class="{{ $currentRoute == 'errors.searcherror.notanswer.index' ? 'bg-slate-800' : '' }} font-bold outline outline-1 px-6 py-2 rounded-lg hover:bg-slate-800 hover:text-white transition duration-200 hover:outline-none" style="{{ $currentRoute == 'errors.searcherror.notanswer.index' ? 'color: white;' : '' }}">
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <h3 class="font-semibold text-[16px]">
                                Category: 
                                <span class="span">{{ $q->category->name }}</span>
                            </h3>
                        </div>
                        <div class="relative cursor-none pointer-events-none">
                            <ion-icon name="heart" class="text-[44px] text-red-primary"></ion-icon>
                            <span class="absolute top-[10px] {{ $q->likes->count() >= 1 ? 'right-[17.5px]' : 'right-[16px]' }} text-white font-semibold">
                                {{ $q->likes->count() }}
                            </span>
                        </div>
                    </div>
                    <p class="text-[18px] text-slate-500 mt-6">
                        {{ Str::limit($q->description_original, 100, "...") }}
                    </p>
