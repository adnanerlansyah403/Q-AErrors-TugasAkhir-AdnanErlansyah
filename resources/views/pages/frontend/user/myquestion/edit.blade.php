...
@section("title", "Edit My Question")
...
<section class="section">
    <div class="container">
        <div class="flex items-center justify-between">
            <p class="text-[18px]"><span class="span font-bold">Cari Masalah</span> / Edit a Question</p>
            <a href="{{ route("users.myquestion.index") }}" class="flex items-center gap-4 bg-red-primary p-4 rounded-lg text-white transition duration-200 w-max">
                Kembali
            </a>
        </div>

        <h1 class="text-lg font-black font-quick-sand mb-10">Ubah Pertanyaanmu</h1>


        <div class="w-full mt-6 flex flex-wrap items-center gap-6">
            {{-- @livewire("form.question-form") --}}
            <form method="POST" action="{{ route("users.myquestion.update", $question) }}" class="card-item gap-6 px-6 py-6 rounded-lg shadow-[rgba(60,_64,_67,_0.3)_0px_1px_2px_0px,_rgba(60,_64,_67,_0.15)_0px_1px_3px_1px] w-full" enctype="multipart/form-data">
                @csrf
                ...
                <div class="flex items-center gap-10 flex-wrap">
                    ...
                    <div class="mb-10 w-[48%]">
                        ...
                        <div class="shadow-[rgba(60,_64,_67,_0.3)_0px_1px_2px_0px,_rgba(60,_64,_67,_0.15)_0px_2px_6px_2px] p-4 rounded-lg active:border border-red-primary transition duration-200">
                            <input type="text" name="title" id="title" class="" placeholder="Your title..." value="{{ $question->title }}" style="width: 100%">
                        </div>
                        @error('title') 
                    ...
                    <div class="mb-10 w-[48%]">
                        ...
                        <select class="shadow-[rgba(60,_64,_67,_0.3)_0px_1px_2px_0px,_rgba(60,_64,_67,_0.15)_0px_2px_6px_2px] p-4 rounded-lg active:border border-red-primary transition duration-200 w-full" name="category" id="category">
                            ...
                            @forelse ($categories as $category)
                                <option value="{{ old('category', $category->id) }}" {{ $category->id == $question->category_id ? "selected" : "" }}>{{ $category->name }}</option>
                            @empty
                    ...
                    <div class="-mt-10 w-full">
                        ...
                        <div class="shadow-[rgba(60,_64,_67,_0.3)_0px_1px_2px_0px,_rgba(60,_64,_67,_0.15)_0px_2px_6px_2px] p-4 rounded-lg active:border border-red-primary transition duration-200 w-full">
                            <textarea name="description" id="description" rows="10" class="w-full">{{ $question->description_editor }}</textarea>
                        </div>
                        @error('description') 
                            <div class="flex items-center gap-2 mt-2">
                                <svg class="h-6 w-6 text-red-primary cursor-pointer"
                    ...
                    <div class="mb-2">
                        <button type="submit" href="#notanswer" class="w-max flex items-center gap-4 font-bold outline outline-1 outline-red-primary px-6 py-4 rounded-lg hover:bg-slate-800 hover:text-white transition duration-200 hover:outline-none">
                            <p><span class="span">Update</span> Masalah</p>
                        </button>
                    </div>
